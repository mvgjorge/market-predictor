#!/usr/bin/env python3
"""
Input: X percent - keep top X percent companies in order of descending market_cap
Download the ticker list from NASDAQ and save in csv.
    ...
    for exchange in ["NASDAQ", "NYSE", "AMEX"]:
        ...
        for _ in range(repeat_times):
            try:
                ...
                temp = url + exchange + "&render=download"
                urlretrieve(temp, 'test.csv')
                print("temp", temp)
                response = urlopen(temp) #url + exchange + '&render=download')
                time.sleep(5)
                ...
                for num, line in enumerate(content):
                    ...
                    print ("values: ", ticker, market_cap)
